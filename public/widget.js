    ...
    try {
        ...
        videos.forEach(video => {
          ...
          if (video.productIds && video.productIds.length > 0) {
              ctaMarkup = `<button class="cta-button" onclick="window.parent.location.href='/products/${video.productIds[0].split('/').pop()}'">Shop Product</button>`;
          }
          ...
          const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
              if (entry.isIntersecting) {
                v.play().catch(() => {
                    // Autoplay might fail if not interacted with
